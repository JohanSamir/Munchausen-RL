...
def maun_target_quantile_values(online_network, target_network,
                           next_states, states, rewards, terminals,
                           num_tau_prime_samples, actions, num_actions, num_quantile_samples,
                           cumulative_gamma, double_dqn, alpha, rng):
  """Build the target for return values at given quantiles.
  ...
  outputs_action = target_network(next_states,
                                    ...
                                    rng=rng1)
  target_quantile_values_action = outputs_action.quantile_values
  _replay_next_target_net_outputs = jnp.squeeze(jnp.mean(target_quantile_values_action, axis=0))

  outputs_action = target_network(states,
                                  num_quantiles=num_quantile_samples,
                                  rng=rng1)
  target_quantile_values_action = outputs_action.quantile_values
  _replay_target_net_outputs = jnp.squeeze(jnp.mean(target_quantile_values_action, axis=0))
  ...
  replay_next_policy = utils.stable_softmax(_replay_next_target_net_outputs, tau, axis=1)
  ...
  munchuasen_term = alpha * tau_log_pi_a
  #########-----------------------------

  #replay_next_policy_ = jnp.tile(replay_next_policy,[num_tau_prime_samples, 1])
  replay_next_policy_ = jnp.tile(replay_next_policy,[num_tau_prime_samples])
  replay_next_log_policy_ = jnp.tile(replay_next_log_policy,[num_tau_prime_samples])

  #########-----------------------------
  batch_size = jnp.shape(_replay.rewards)[0]

  _replay_net_target_outputs = target_convnet(next_states, num_tau_prime_samples)
  replay_net_target_quantile_values = _replay_net_target_outputs.quantile_values

  replay_quantile_values = jnp.reshape(
      _replay_net_target_quantile_values,
      [batch_size * num_tau_prime_samples, num_actions])
  ...
  modified_bellman =  rewards + gamma_with_terminal * target_quantile_values
  return rng, jax.lax.stop_gradient(modified_bellman)
